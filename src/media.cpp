...
bool media::setSectorBuffer( uint8_t * d88rwBuffer )
{
  ...
  m_d88FileHeader = (PD88_FILE_HEADER)malloc( size );
...
bool media::insert( bool writeProtected, const char * imgFileName )
{
  ...
  memset( m_imgFileName, 0, sizeof( m_imgFileName ) );
  strcpy( m_imgFileName, imgFileName );
...
bool media::formatSector( int cylinder, int head, int sectorNo, int sectorLen, int eot, uint8_t fillData, int length )
{
  ...
  PD88_FILE_SECTOR pD88SectorHead = (PD88_FILE_SECTOR)&m_d88rwBuffer[0];
  ...
  bool format = true;
  for ( int secNo = sectorNo; secNo <= eot; secNo++ )
  {
    pD88SectorHead->sector = secNo;
    size_t result = fwrite( &m_d88rwBuffer[0], 1, d88SectorSize, m_formatfp );
    if ( result != d88SectorSize )
    {
      format = false;
      break;
